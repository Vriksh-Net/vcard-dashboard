...
import { useEffect, useState } from "react"
import { Facebook, Instagram, Linkedin, Mail, MapPin, Phone, Twitter, Youtube, Download, Globe } from "lucide-react"
import { Button } from "@/components/ui/button"
    ...
    const fetchVCard = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch vCard data: ${response.status} - ${response.statusText}`)
        }
        ...
        // Ensure socialLinks is always an array
        if (!data.socialLinks) {
          console.warn("No socialLinks property found in response, initializing empty array")
          data.socialLinks = []
        } else if (!Array.isArray(data.socialLinks)) {
          console.warn("socialLinks is not an array, converting to array:", data.socialLinks)
          // If socialLinks is not an array, convert it to an array
          const links = []
        ...
        console.log("Social links processed:", data.socialLinks.length)
        data.socialLinks.forEach((link: { platform: any; url: any }, index: number) => {
          console.log(`Social link ${index + 1}: ${link.platform} - ${link.url}`)
        })
            ...
            <div className="mt-4">
              ...
              {showDebug && (
                <div className="mt-2 text-left bg-gray-100 p-4 rounded-md overflow-auto max-h-96">
                  ...
                  {debugInfo.sampleIds && debugInfo.sampleIds.length > 0 && (
                    <div className="mt-2">
                      <p className="font-semibold">Available vCard IDs in database:</p>
                      <ul className="list-disc pl-5">
                        {debugInfo.sampleIds.map((id, index) => (
                          <li key={index} className="break-all">
                            <a href={`/vcard/${id}`} className="text-blue-600 hover:underline">
                              {id}
                            </a>
                          </li>
                        ))}
                      </ul>
                      <p className="mt-2 text-sm">Click on an ID above to try viewing that vCard instead.</p>
                    </div>
                  )}
                </div>
              )}
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <div className="flex flex-col items-center -mt-16 z-10 px-4">
        ...
        <h1 className="text-2xl font-bold text-center">{vCard.name}</h1>
        {vCard.position && <p className="text-gray-600 text-center">{vCard.position}</p>}
        {vCard.company && <p className="text-gray-600 text-center">{vCard.company}</p>}
      </div>
      ...
      <div className="px-4 mt-6 space-y-2">
        ...
        {vCard.website && (
          <a href={vCard.website} target="_blank" rel="noopener noreferrer" className="block">
            <Button
              className="w-full py-6 text-white flex items-center justify-center gap-2"
              style={{ backgroundColor: primaryColor }}
            >
      ...
      <div className="px-4 mt-8 space-y-6">
        ...
        <div className="flex items-start gap-3 border-b pb-4">
          ...
          <div>
            <p className="text-sm text-gray-500">Email</p>
            <a href={`mailto:${vCard.email}`} className="text-gray-800 break-all">
              {vCard.email}
            </a>
          </div>
        </div>
        ...
        {vCard.website && (
          <div className="flex items-start gap-3 border-b pb-4">
            ...
            <div>
              <p className="text-sm text-gray-500">Website</p>
              <a href={vCard.website} target="_blank" rel="noopener noreferrer" className="text-gray-800 break-all">
                {vCard.website}
              </a>
            </div>
          </div>
