...
import { useRouter } from "next/navigation"
import Link from "next/link"
import { ArrowLeft, Save, Upload, Check, Copy, LinkIcon, ChevronLeft, ChevronRight } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Card, CardContent } from "@/components/ui/card"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
...
export default function NewCardPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("Social links to be submitted:", JSON.stringify(socialLinksArray, null, 2))
      ...
      // Verify social links were returned
      if (data.socialLinks) {
        console.log("Social links in response:", JSON.stringify(data.socialLinks, null, 2))
      } else {
        console.warn("No social links in response!")
      }
      ...
      toast({
        ...
        description: (
          <div>
            ...
            <p className="mt-2">
              <strong>vCard Link:</strong>{" "}
              <a href={newVCardLink} className="underline" target="_blank" rel="noopener noreferrer">
                {newVCardLink}
              </a>
            </p>
            <p className="mt-2 text-sm">Use this link with your NFC tool.</p>
      ...
      console.error("Error creating vCard:", error)
      const errorAsError = error as Error
      setError(errorAsError.message || "There was a problem creating your vCard. Please try again.")
      toast({
        title: "Error Creating vCard",
        description: errorAsError.message || "There was a problem creating your vCard. Please try again.",
        style: { backgroundColor: "red", color: "white" },
        className: "bg-red-500 text-white",
      })
    } finally {
  ...
  const testVCardLink = async () => {
    ...
    try {
      const response = await fetch(vCardLink.replace(window.location.origin, ""))
      const status = response.status
  ...
  return (
    <div className="grid gap-4 md:gap-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Create New vCard</h1>
          <p className="text-muted-foreground">Add contact information to create a new digital vCard</p>
        </div>
      </div>
      ...
      {vCardCreated && (
        <Alert className="bg-green-50 border-green-200 text-green-900">
          ...
          <AlertTitle>vCard Created Successfully</AlertTitle>
          <AlertDescription className="flex flex-col gap-2">
            <p>Your vCard has been created and is ready to be shared or programmed to an NFC card.</p>
            <div className="flex items-center gap-2 mt-2">
              <Input value={vCardLink} readOnly className="flex-1" />
              <Button size="sm" onClick={handleCopyLink}>
            ...
            <div className="flex gap-2 mt-2">
              ...
              {vCardData && (
                <Button variant="outline" size="sm" onClick={() => router.push(`/dashboard/cards/${vCardData.id}/nfc`)}>
                  Configure NFC
                </Button>
              )}
            </div>
        ...
        <div className="space-y-4">
          <Tabs defaultValue="template" className="w-full">
            ...
            <form onSubmit={handleSubmit}>
              <TabsContent value="template" className="space-y-4 pt-4">
                <div className="space-y-4">
                  <h3 className="text-lg font-medium">Choose a Template Design</h3>
                  <p className="text-sm text-muted-foreground">
                    Select a template design for your vCard. This will determine how your vCard appears when shared.
                  </p>
                      ...
                      <div className="flex space-x-2">
                        {templates.map((template, index) => (
                          <button
                            ...
                            type="button"
                            className={`h-2.5 w-2.5 rounded-full ${
                              currentTemplateIndex === index ? "bg-primary" : "bg-gray-300"
                            }`}
                            onClick={() => {
                              setCurrentTemplateIndex(index)
                              handleTemplateChange(template.id)
                  ...
                  <div className="mt-4">
                    <Label>Template Name</Label>
                    <p className="text-sm font-medium">{templates[currentTemplateIndex].name}</p>
                  </div>
                </div>
              </TabsContent>
            ...
            <CardContent className="p-6">
              ...
              <div className="mt-6 rounded-lg border p-4">
                {formData.name ? (
                  <div className="relative h-[400px] w-full max-w-[250px] mx-auto overflow-hidden rounded-lg border">
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center -mt-8 z-10 px-4">
                        ...
                        <h3 className="text-sm font-bold text-center">{formData.name}</h3>
                        {formData.position && <p className="text-xs text-gray-600 text-center">{formData.position}</p>}
                        {formData.company && <p className="text-xs text-gray-600 text-center">{formData.company}</p>}
                      </div>
                      ...
                      <div className="px-4 mt-4 space-y-2 text-xs">
                        <div className="flex items-start gap-1 border-b pb-1">
                          <div>
                            <p className="text-xs text-gray-500">Phone</p>
                            <p className="text-gray-800">{formData.phone || "Your phone number"}</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-start gap-1 border-b pb-1">
                          <div>
                            <p className="text-xs text-gray-500">Email</p>
                            <p className="text-gray-800 break-all">{formData.email || "Your email"}</p>
                          </div>
                        </div>
                        ...
                        {(formData.position || formData.company) && (
                          <div className="flex items-start gap-1 border-b pb-1">
                            <div>
                              <p className="text-xs text-gray-500">Position</p>
                              <p className="text-gray-800">{formData.position || "Your position"}</p>
                              <p className="text-gray-800">{formData.company || "Your company"}</p>
                            </div>
                          </div>
                        )}
                      ...
                      {/*//! Social Media */}
                      {Object.values(formData.socialLinks).some((link) => link.trim() !== "") && (
                        <div className="px-4 mt-4">
                          <h2 className="text-center text-xs text-gray-500 mb-2">Social Media</h2>
                          <div className="flex justify-center gap-2">
                            {Object.entries(formData.socialLinks).map(([platform, url]) => {
                              ...
                              return (
                                ...
                                >
                                  <span className="text-[8px]">{platform.charAt(0).toUpperCase()}</span>
                                </div>
                              )
                            })}
                          </div>
                  ...
                  <div className="flex h-[300px] items-center justify-center text-center">
                    <div className="space-y-2">
                      <p className="text-sm text-muted-foreground">Fill out the form to see a preview of your vCard</p>
                    </div>
                  </div>
                )}
              </div>
